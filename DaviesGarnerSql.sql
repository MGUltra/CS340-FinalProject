...
CREATE TABLE `excercise_muscle_groups` (
`e_id` int(11) NOT NULL,
...
INSERT INTO `excercise_muscle_groups` (e_id, mg_id) VALUES
...
((SELECT id from workout where did = (SELECT id from date where date = [date])), (SELECT id from exercise where exercise_name=[exerciseName]));
